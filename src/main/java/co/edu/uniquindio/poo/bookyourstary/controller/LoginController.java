...
import co.edu.uniquindio.poo.bookyourstary.util.LoginHelper;
...
public class LoginController {
        ...
        try {
            ...
            if (session != null) {
                ...
                if (usuario instanceof Client) {
                    ...
                    MainController.showAlert("Inicio de sesión", "Bienvenido administrador",
                            Alert.AlertType.INFORMATION);
                    MainController.loadScene("MenuAdmin", 900, 600);
                }
            } else {
                System.err.println("Error de autenticación: credenciales inválidas");
                MainController.showAlert("Error de inicio de sesión", "Usuario o contraseña incorrectos",
                        Alert.AlertType.ERROR);
            }
        } catch (Exception e) {
            System.err.println("Error en handleLogin: " + e.getMessage());
            e.printStackTrace();
            MainController.showAlert("Error de inicio de sesión",
                    "Ocurrió un error al procesar la solicitud: " + e.getMessage(), Alert.AlertType.ERROR);
        }
    }
    ...
    public void handlePasswordRecovery(String email) {
        ...
        if (email == null || !email.matches("^[A-Za-z0-9+_.-]+@[A-Za-z0-9.-]+$")) {
            MainController.showAlert("Recuperación de contraseña", "Por favor ingrese un correo válido.",
                    Alert.AlertType.WARNING);
            return;
        }
        try {
            MainController.getInstance().getCodeRecoveryService().recoverPassword(email);
            MainController.showAlert("Recuperación de contraseña",
                    "Revise su correo electrónico para el código de recuperación.", Alert.AlertType.INFORMATION);
        } catch (Exception e) {
            MainController.showAlert("Error", "No se pudo enviar el correo de recuperación. Intente más tarde.",
                    Alert.AlertType.ERROR);
        }
    }
