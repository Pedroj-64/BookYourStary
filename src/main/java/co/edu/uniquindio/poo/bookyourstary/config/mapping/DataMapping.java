...
public class DataMapping {
    public static Admin createTestAdmin() {
        ...
        if (existingAdmin != null) {
            ...
            System.out.println(
                    "NUEVO Admin creado y guardado con ID: " + admin.getId() + " y email: " + admin.getEmail());
            return admin;
        }
    }
    ...
    public static void createAllHostings() {
        try {
            ...
            // Verificamos primero si ya existen hostings en el sistema
            List<Hosting> existingHostings = MainController.getInstance().getHostingService().findAllHostings();
            if (existingHostings != null && !existingHostings.isEmpty()) {
                System.out.println(
                        "Ya existen " + existingHostings.size() + " hostings en el sistema. No se crearán nuevos.");
                return;
            }
            ...
            City medellin = new City("Medellín", "Colombia", "Antioquia");
            ...
            roomsHotelLuna.add(new Room("202", 120000, 2, "FotoHotelRelleno.png", "Habitación doble premium"));
            ...
            MainController.getInstance().getAdminService().createApartament(
                    "Apartamento Moderno", medellin, "Apartamento céntrico", "FotoApartamentoRelleno.png",
                    150000, 3, new LinkedList<>(serviciosBasicos),
                    LocalDate.now(), LocalDate.now().plusDays(30));

            MainController.getInstance().getAdminService().createApartament(
                    "Apartamento Familiar", bogota, "Apartamento espacioso", "FotoApartamentoRelleno.png",
                    200000, 4, new LinkedList<>(serviciosPremium),
                    LocalDate.now(), LocalDate.now().plusDays(30));

            MainController.getInstance().getAdminService().createApartament(
                    "Apartamento Vista", armenia, "Apartamento con vista", "FotoApartamentoRelleno.png",
                    180000, 2, new LinkedList<>(serviciosBasicos),
                    LocalDate.now(), LocalDate.now().plusDays(30));

            System.out.println("Creación de hostings de prueba completada con éxito.");
        } catch (Exception e) {
