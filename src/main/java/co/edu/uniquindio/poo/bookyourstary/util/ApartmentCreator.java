...
/**
 * Clase utilitaria para crear apartamentos y evitar problemas de compatibilidad con DataMapping
 */
public class ApartmentCreator {

    /**
     * Crea y guarda apartamentos directamente en el repositorio
     * Este método se ofrece como alternativa por si hay problemas con la interfaz estándar
     */
    public static void createApartments() {
        try {
            ...
            List<Apartament> existingApartaments = MainController.getInstance().getHostingService().getApartamentRepository().findAll();
            if (existingApartaments != null && !existingApartaments.isEmpty()) {
                System.out.println("Ya existen " + existingApartaments.size() + " apartamentos en el sistema. No se crearán nuevos.");
                return;
            }
